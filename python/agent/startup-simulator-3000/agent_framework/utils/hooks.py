...
class ToolContext:
    ...
    inputs: Dict[str, Any] = field(
        metadata={"description": "Input parameters for the tool"}
    )
    previous_tools: List[str] = field(
        metadata={
            "description": "List of tools that were previously executed in this task"
        }
    )
    previous_results: List[Dict[str, Any]] = field(
        metadata={"description": "Results from previous tool executions"}
    )
    previous_errors: List[str] = field(
        metadata={"description": "Errors from previous tool executions"}
    )
    message_history: List[Dict[str, Any]] = field(
        metadata={"description": "Complete history of messages and tool executions"}
    )
    agent_id: str = field(
        metadata={"description": "ID of the agent executing the tool"}
    )
    ...
    metadata: Dict[str, Any] = field(
        metadata={"description": "Additional metadata from agent configuration"}
    )
