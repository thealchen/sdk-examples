...
class ToolMetadata(BaseModel):
    ...
    examples: Optional[List[Dict[str, Any]]] = Field(
        default=None, description="Example uses of the tool"
    )
...
class Tool:
    ...
    description: str = field(
        metadata={"description": "Human-readable description of what the tool does"}
    )
    tags: List[str] = field(
        metadata={"description": "Categories or labels for the tool's capabilities"}
    )
    input_schema: Dict[str, Any] = field(
        metadata={"description": "JSON Schema defining expected input parameters"}
    )
    output_schema: Dict[str, Any] = field(
        metadata={"description": "JSON Schema defining the tool's output structure"}
    )
...
class ToolSelectionCriteria(BaseModel):
    ...
    required_tags: List[str] = Field(
        default_factory=list, description="Tags that a tool must have to be considered"
    )
...
class ToolCall(BaseModel):
    ...
    outputs: Optional[Dict[str, Any]] = Field(
        default=None, description="Results returned by the tool execution"
    )
    ...
    success: bool = Field(
        default=True, description="Whether the tool execution completed successfully"
    )
...
class ExecutionStep(BaseModel):
    ...
    tool_calls: List[ToolCall] = Field(
        default_factory=list, description="Tools that were called during this step"
    )
...
class TaskExecution(BaseModel):
    ...
    output: Optional[str] = Field(
        default=None, description="Final result or response from the task execution"
    )
    ...
    error: Optional[str] = Field(
        default=None, description="Error message if the task execution failed"
    )
