...
def convert_text_to_arithmetic_expression(text):
    ...
    response = openai_client.chat.completions.create(
        model="gpt-4o",
        messages=[{"role": "user", "content": f'Convert the expression {text} with text numbers to a valid arithmetic expression. Respond with only the arithmetic expression.'}],
    )
